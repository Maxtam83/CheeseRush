    ...
    void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
    {
        ...
        rimWidth = max(lerp(rimWidth, rimWidth * lerp(0, 1, poiLight.lightMap - float(0.172)) * float(8.46), float(0.178)), 0);
        rim = 1 - smoothstep(min(float(0.849), rimWidth), rimWidth, viewDotNormal);
